...
# Añadir intent de navegador https al <queries> si no está ya presente
MANIFEST=android/app/src/main/AndroidManifest.xml
if ! grep -q 'android:scheme="https"' "$MANIFEST"; then
  # Insertar antes del cierre </queries>
  if grep -q '</queries>' "$MANIFEST"; then
    sed -i 's#</queries>#    <intent>\n            <action android:name="android.intent.action.VIEW" />\n            <data android:scheme="https" />\n        </intent>\n    </queries>#' "$MANIFEST"
    echo "Query https añadida al manifest."
  else
    echo "AVISO: no hay bloque <queries> en el manifest. Añádelo a mano antes de </manifest>:"
    echo '    <queries><intent><action android:name="android.intent.action.VIEW"/><data android:scheme="https"/></intent></queries>'
  fi
else
  echo "El manifest ya tiene la query https."
fi
echo "v38.4.0: Acerca de + Ko-fi aplicado."
